...
\end{itemize}
For the dimension-dependence experiments, we extend the Gaussian mixture to $d$ dimensions, placing the 8 centers on a circle in the first two coordinates.
...
We sample 256 pairs from $p_0 \times \hat{p}_{\mathrm{data}}$ and plot the distribution of cosine similarities at 7 time steps, for each of our three 2D datasets.
As expected, the histograms concentrate near~1 as $t$ increases, confirming the collapse to a single conditional velocity.
On 2D data, the transition occurs at intermediate $t$ ($\approx 0.4$--$0.6$), consistent with the paper's 2-moons experiments.

\paragraph{Dimension dependence (Figure~1c).}
Using the $d$-dimensional Gaussian mixture with $d \in \{2, 10, 50, 100, 500, 1000\}$ and $n = 2000$, we plot the fraction of pairs with $\cos > 0.9$ vs.\ $t$.
As dimension increases, the curve shifts sharply to the left: at $d = 1000$, near-perfect alignment occurs already at $t \approx 0.02$.

\paragraph{Comparison with Biroli et al.}
We extract the empirical collapse time for each dimension and compare with the theoretical scaling $t_C \propto \log(n)/d$.
We also fix $d = 50$ and vary $n$ to verify the $\log(n)$ factor.
Both scalings are well matched, confirming that the Biroli et al.\ prediction carries over to the flow matching setting.

\subsection{Replication of Figure~2: velocity approximation failure}\label{subsec:fig2}

We train flow matching models with fixed capacity (256-hidden MLP) on the Gaussian mixture dataset with $n \in \{10, 50, 200, 1000, 5000\}$.
For each model, we evaluate the approximation error $\norm{u_\theta - \hat{u}^\star}^2$ across time.

With $n = 10$ (darkest curve), the error is nearly zero: the network perfectly memorizes $\hat{u}^\star$.
As $n$ grows, the error increases, with pronounced peaks near $t \approx 0.15$ and $t \to 1$.
In parallel, the nearest-neighbor distance grows with $n$, confirming that memorization gives way to generalization as the dataset becomes too complex for the network.
...
Starting from 256 fixed noise vectors, we measure the mean nearest-neighbor distance to the training set for each $\tau$.

The curve shows a clear plateau for $\tau \leq 0.2$ (same generalization as pure $u_\theta$), then a sharp drop.
By $\tau = 0.4$, the generated samples are nearly indistinguishable from training data.
...
We repeat the hybrid experiment (Section~\ref{subsec:fig3}) on all three 2D datasets.
The switching time varies across geometries: \textbf{moons} (more structured, lower intrinsic dimension) shows a slightly earlier switching point than the more spread-out \textbf{rings} or the multi-modal \textbf{Gaussian mixture}.
...
Using the $d$-dimensional Gaussian mixture with $d \in \{2, 10, 50, 100\}$ and a fixed MLP capacity, we observe that:
\begin{itemize}[nosep,leftmargin=1.5em]
    \item The collapse time decreases with $d$, consistent with the $\log(n)/d$ scaling.
    \item The switching time also shifts to earlier $\tau$ in higher dimensions, because the non-stochastic phase starts sooner and the network has less time to ``inject'' generalization.
    \item In very high dimensions ($d = 100$), the network struggles more with $\hat{u}^\star$ overall, and the gap between the hybrid curve at $\tau = 0$ and $\tau = 0.1$ is already small.
\end{itemize}

\subsection{Model capacity}

Fixing the Gaussian mixture at $d = 2$, $n = 2000$, we vary the MLP hidden dimension in $\{32, 64, 128, 256, 512\}$.
...
We train the same architecture for $\{1000, 5000, 10000, 20000, 50000\}$ gradient steps.
...
At $50000$ steps, the switching time approaches that of a well-converged model, with little further change --- the bottleneck is capacity, not optimization.
...
\paragraph{Inconclusive aspects.}
We did not observe a significant impact of alternative time schedules (e.g., cosine vs.\ linear) on the switching time for our toy datasets.
This may be because the schedules primarily affect the density of Euler steps across different phases of the flow, and our toy problems are simple enough that 200 Euler steps suffice regardless of scheduling.
A more careful study on image-scale data would be needed.

\paragraph{Critical perspective on the paper.}
While the paper convincingly rules out target stochasticity as the source of generalization, we note that the equivalence between CFM and FM losses ($\nabla_\theta \cL_{\mathrm{CFM}} = \nabla_\theta \cL_{\mathrm{FM}}$) already implies that both losses share the same minimizer $\hat{u}^\star$.
The novelty of the EFM algorithm (Section~\ref{sec:efm}) is therefore not in changing the \emph{optimal} velocity but in changing the \emph{variance of the training signal}, which may affect optimization dynamics and implicit regularization.
The paper's main contribution is empirical: showing that the variance reduction does not hurt generalization, which is a stronger statement than the theoretical equivalence alone.

%==========================================================================
\section{Use of chatbot tools}\label{sec:tools}
%==========================================================================

We used a large language model (Claude) as a coding assistant throughout this project:
\begin{itemize}[leftmargin=1.5em]
    \item \textbf{Code generation:} We used it to implement the full codebase (data loaders, models, training loops, metrics, and notebooks) from skeleton files. The generated code was correct on first pass for all modules except a broadcasting bug in the softmax weight computation, which was caught by our unit tests and fixed immediately.
    \item \textbf{Mathematical writing:} The LLM helped structure and typeset the mathematical sections of this report, particularly the diffusion/FM equivalence proof sketch. We verified all equations against the course notes and the original paper.
    \item \textbf{Limitations encountered:} The model occasionally used imprecise mathematical language (e.g., conflating ``gradient equivalence'' with ``loss equivalence'') which we corrected manually. It also initially generated placeholder text for experimental results, which we replaced with actual findings after running the notebooks.
\end{itemize}
...
All figures referenced in this report are generated by the companion Jupyter notebooks:
\begin{itemize}[nosep,leftmargin=1.5em]
    \item \texttt{01\_figure1\_stochasticity.ipynb} --- Figures for Section~\ref{subsec:fig1}.
    \item \texttt{02\_figure2\_velocity\_approx.ipynb} --- Figures for Section~\ref{subsec:fig2}.
    \item \texttt{03\_figure3\_generalization\_time.ipynb} --- Figures for Sections~\ref{subsec:fig3} and~\ref{sec:additional}.
    \item \texttt{04\_image\_experiments.ipynb} --- CFM vs.\ EFM on Fashion-MNIST.
\end{itemize}
The notebooks are self-contained and designed to run on a free Google Colab T4 GPU.
Generated PDF figures are saved alongside each notebook.
